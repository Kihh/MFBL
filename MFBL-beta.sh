...
Ngrokmenu() {
    ...
    1)
        clear
        echo "此处考虑到中国服务器下载慢，仅使用cloudflare代理了下载链接而已，非魔改过的软件请勿造谣！"
        wget --no-check-certificate https://npdp.pages.dev/c/4VmDzA7iaHb/ngrok-stable-linux-amd64.zip -O ngrok-stable-linux-amd64.zip
        apt install unzip -y
